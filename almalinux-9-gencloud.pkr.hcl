...
source "qemu" "almalinux-9-gencloud-aarch64" {
  ...
  ssh_password       = var.gencloud_ssh_password
  ssh_timeout        = var.ssh_timeout
  boot_command       = local.aarch64_gencloud_boot_command_9
  boot_wait          = var.boot_wait
...
source "qemu" "almalinux-9-gencloud-s390x" {
  ...
  qemuargs = [
    ...
    ["-append", "ro ramdisk_size=40000 console=ttysclp0 ip=dhcp inst.text inst.ks=http://{{ .HTTPIP }}:{{ .HTTPPort }}/almalinux-9.gencloud-s390x.ks"],
...
build {
  ...
  provisioner "ansible" {
    ...
    except = ["qemu.almalinux-9-gencloud-s390x"]
  }
}
