# AlmaLinux OS 9 Packer template for Cloud-init included and OpenStack compatible Generic Cloud (ext4) images

source "qemu" "almalinux-9-gencloud-ext4-x86_64" {
  ...
  boot_command       = var.gencloud_ext4_boot_command_9_x86_64
  ...
  vm_name            = "AlmaLinux-9-GenericCloud-ext4-${var.os_ver_9}-${formatdate("YYYYMMDD", timestamp())}.x86_64.qcow2"
  cpu_model          = "host"
  cpus               = var.cpus
  efi_boot           = true
...
source "qemu" "almalinux-9-gencloud-ext4-aarch64" {
  ...
  ssh_password       = var.gencloud_ssh_password
  ssh_timeout        = var.ssh_timeout
  boot_command       = local.aarch64_gencloud_ext4_boot_command_9
  boot_wait          = var.boot_wait
  ...
  net_device         = "virtio-net"
  qemu_binary        = var.qemu_binary
  vm_name            = "AlmaLinux-9-GenericCloud-ext4-${var.os_ver_9}-${formatdate("YYYYMMDD", timestamp())}.aarch64.qcow2"
...
source "qemu" "almalinux-9-gencloud-ext4-ppc64le" {
  ...
  net_device         = "virtio-net"
  qemu_binary        = var.qemu_binary
  vm_name            = "AlmaLinux-9-GenericCloud-ext4-${var.os_ver_9}-${formatdate("YYYYMMDD", timestamp())}.ppc64le.qcow2"
  cpus               = var.cpus
}
