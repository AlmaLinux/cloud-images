  ...
  post-processors {
    ...
    post-processor "shell-local" {
      inline = [
        ...
        "cd ${source.name}",
        "tar --use-compress-program='gzip -9' -cvf ../AlmaLinux-10-Vagrant-hyperv-${var.os_ver_10}-${formatdate("YYYYMMDD", timestamp())}.${var.build_number}.x86_64.box .",
        "mv ../output-${source.name}/${source.name}.raw ../AlmaLinux-10-Vagrant-hyperv-${var.os_ver_10}-${formatdate("YYYYMMDD", timestamp())}.${var.build_number}.x86_64.raw"
      ]
      only = [
        "qemu.almalinux_10_vagrant_hyperv_x86_64",
      ]
    ...
    post-processor "shell-local" {
      inline = [
        ...
        "qemu-img convert -f raw -O vhdx output-${source.name}/${source.name}.raw '${source.name}/Virtual Hard Disks/almalinux.vhdx'",
        "cd ${source.name}",
        "tar --use-compress-program='gzip -9' -cvf ../AlmaLinux-10-Vagrant-hyperv-${var.os_ver_10}-${formatdate("YYYYMMDD", timestamp())}.${var.build_number}.x86_64_v2.box .",
        "mv ../output-${source.name}/${source.name}.raw ../AlmaLinux-10-Vagrant-hyperv-${var.os_ver_10}-${formatdate("YYYYMMDD", timestamp())}.${var.build_number}.x86_64_v2.raw"
      ]
      only = [
        "qemu.almalinux_10_vagrant_hyperv_x86_64_v2",
      ]
