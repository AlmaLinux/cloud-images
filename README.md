...
1. Use one of these methods to set up your AWS API credentials:
...
2. By default, `us-east-1` is set as a region of:
- Source AMI: `aws_source_ami_[8-9]_(x86_64|aarch64)`
- Builder EC2 Instance created from Source AMI: `aws_ami_region`
- The list of regions to copy the AMI to: `aws_ami_regions`

If you build for different region than `us-east-1`, Use the one of the methods listed below to set input variables:
...
Set this input variable for different region than `us-east-1` : `aws_source_ami_8_x86_64`
```sh
...
Set this input variable for different region than `us-east-1` : `aws_source_ami_8_aarch64`
```sh
...
Set this input variable for different region than `us-east-1` : `aws_source_ami_9_x86_64`
```sh
packer build -only=amazon-ebssurrogate.almalinux_9_ami_x86_64 .
```
...
Set this input variable for different region than `us-east-1` : `aws_source_ami_9_aarch64`
```sh
...
Note: Only Nitro based instances are supported
