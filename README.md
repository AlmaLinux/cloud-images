...
| [VAGRANT_BUILD_TEST_PUBLISH.md](VAGRANT_BUILD_TEST_PUBLISH.md) | `vagrant-build-test-publish.yml`, `shared-steps/`, `vagrant-publish-steps/action.yml` | Unified single-dispatch Vagrant pipeline: build → `vagrant up` test → publish each box to the HCP Vagrant Registry in-job on the build runner (Hyper-V is build-only; publishes run in parallel with retry-on-collision) |
...
| [AWS_BUILD_TEST_COPY_RELEASE.md](AWS_BUILD_TEST_COPY_RELEASE.md) | `aws-build-test-copy-release.yml` | Unified single-dispatch AWS pipeline: build → test → copy to all regions + one `almalinux/wiki` PR → release to Marketplace, chained on job results (a failed test skips copy/release) |
| [AZURE_BUILD_RELEASE_TEST_PUBLISH.md](AZURE_BUILD_RELEASE_TEST_PUBLISH.md) | `azure-build-release-test-publish.yml`, `azure-gallery-steps/`, `azure-test-steps/`, `azure-marketplace-steps/action.yml` | Unified single-dispatch Azure pipeline: build → release to Compute Gallery (from the local `.raw`, no S3 round-trip) → test → publish to Marketplace, each stage gated by an input |
...
| [AZURE_TEST.md](AZURE_TEST.md) | `azure-test.yml` | Sanity-test an Azure Compute Gallery image version: launch a fresh VM from the gallery image, run release/arch/disk/`dnf` assertions over SSH, collect the package list, tear down VM + auto-created peers |
...
| [OCI_BUILD_RELEASE_TEST_PUBLISH.md](OCI_BUILD_RELEASE_TEST_PUBLISH.md) | `oci-build-release-test-publish.yml`, `oci-compute-image-steps/`, `oci-test-steps/`, `oci-marketplace-steps/action.yml` | Unified single-dispatch OCI pipeline: build → release to a Compute Image (from the local `.qcow2`, no S3 round-trip) → test → publish to Marketplace listings, each stage gated by an input |
...
| [GENCLOUD_BUILD_TEST.md](GENCLOUD_BUILD_TEST.md) | `gencloud-build-test.yml`, `shared-steps/`, `gencloud-test-steps/action.yml` | Unified single-dispatch GenericCloud pipeline: build → boot-test each image in-job under QEMU/KVM on the build runner (aarch64 builds on `ubuntu24-full-arm64` so the test runs locally; no S3 round-trip) |
...
| [OPENNEBULA_BUILD_TEST.md](OPENNEBULA_BUILD_TEST.md) | `opennebula-build-test.yml`, `shared-steps/`, `opennebula-test-steps/action.yml` | Unified single-dispatch OpenNebula pipeline: build → boot-test each image in-job under QEMU/KVM on the build runner with a one-context `CONTEXT` ISO (aarch64 builds on `ubuntu24-full-arm64` so the test runs locally; no S3 round-trip) |
| [OPENNEBULA_TEST.md](OPENNEBULA_TEST.md) | `opennebula-test.yml`, `opennebula-test-steps/action.yml` | Sanity-test an OpenNebula `.qcow2` image directly under QEMU/KVM on the runner with a one-context `CONTEXT` ISO, run release/arch/disk/`dnf` and one-context-specific assertions over SSH, collect the package list, shut the guest down |
