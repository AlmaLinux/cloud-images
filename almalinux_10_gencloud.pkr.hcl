...
source "qemu" "almalinux_10_gencloud_aarch64" {
  ...
  ssh_password       = var.gencloud_ssh_password
  ssh_timeout        = var.ssh_timeout
  boot_command       = local.aarch64_gencloud_boot_command_10
  boot_wait          = var.boot_wait
...
source "qemu" "almalinux_10_gencloud_ppc64le" {
  ...
  ssh_password       = var.gencloud_ssh_password
  ssh_timeout        = var.ssh_timeout
  boot_command       = local.gencloud_boot_command_10_ppc64le
...
source "qemu" "almalinux_10_gencloud_s390x" {
  ...
  net_device         = "virtio-net"
  qemu_binary        = var.qemu_binary
  vm_name            = "AlmaLinux-10-GenericCloud-${var.os_ver_10}-${formatdate("YYYYMMDD", timestamp())}.${var.build_number}.s390x.qcow2"
  cpus               = var.cpus
  qemuargs = [
    ["-cpu", "max"],
    ["-kernel", "${var.s390x_boot_dir}/kernel.img"],
    ["-initrd", "${var.s390x_boot_dir}/initrd.img"],
    ["-append", "ro ramdisk_size=40000 console=ttysclp0 ip=dhcp inst.text inst.ks=http://{{ .HTTPIP }}:{{ .HTTPPort }}/almalinux-10.gencloud-s390x.ks"],
...
build {
  ...
  provisioner "ansible" {
    ...
    except = ["qemu.almalinux_10_gencloud_s390x"]
  }
}
