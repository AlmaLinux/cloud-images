# AlmaLinux OS 10 Packer template for Azure VM images

source "qemu" "almalinux_10_azure_x86_64" {
  ...
  boot_command       = var.azure_boot_command_10_x86_64
...
source "qemu" "almalinux_10_azure_aarch64" {
  ...
  ssh_password       = var.gencloud_ssh_password
  ssh_timeout        = var.ssh_timeout
  boot_command       = var.azure_boot_command_10_aarch64
  boot_wait          = var.boot_wait
  accelerator        = "kvm"
...
source "qemu" "almalinux_10_azure_64k_aarch64" {
  iso_url            = local.iso_url_10_aarch64
  iso_checksum       = local.iso_checksum_10_aarch64
  http_directory     = var.http_directory
  shutdown_command   = var.root_shutdown_command
  ...
  ssh_password       = var.gencloud_ssh_password
  ssh_timeout        = var.ssh_timeout
  boot_command       = var.azure_boot_command_10_64k_aarch64
  boot_wait          = var.boot_wait
  accelerator        = "kvm"
  firmware           = var.aavmf_code
  use_pflash         = false
  ...
  net_device         = "virtio-net"
  qemu_binary        = var.qemu_binary
  vm_name            = "AlmaLinux-10-Azure-${var.os_ver_10}-${formatdate("YYYYMMDD", timestamp())}.${var.build_number}-64k.aarch64.raw"
  cpu_model          = "host"
  cpus               = var.cpus
  qemuargs = [
    ["-boot", "strict=on"],
